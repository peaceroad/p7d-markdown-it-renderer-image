...
1. Initialize options (remote sizing on, remoteTimeout 5000ms, remoteMaxBytes 16MB, cacheMax 64, suppressErrors none, hideTitle true, resizeDataAttr empty by default).
...
3. For each `image` token:
   ...
   - Set final `src`/`alt`/`title` (title removed only when `hideTitle` + `resize` + resize-pattern match). When removed, preserve the resize hint in `resizeDataAttr` only if configured (opt-in). Optional `decoding`/`loading`.
...
3. For each DOM `img`:
   ...
   - Set `alt`, `title` (or remove if hideTitle and resize/title match); when removed, store the resize hint in `resizeDataAttr` for later DOM updates; when title is kept or non-resize, clear `resizeDataAttr`; apply `loading`/`decoding` defaults if absent.
